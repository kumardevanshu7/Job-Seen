...
3. **Purana data migrate hoga automatically:** Jab purane users login karenge, `getUserProfile()` function automatically unka data `publicProfiles` aur `usernames` collections mein migrate kar dega. Lekin agar aapke paas already bahut saara production data hai, ek backfill script chalana safer hoga.
4. **App Check optional hai:** Agar aap `PUBLIC_FIREBASE_APPCHECK_SITE_KEY` set nahi karte, toh App Check simply skip ho jaata hai — koi error nahi aayega. Isko enable karne ke liye Firebase Console mein reCAPTCHA Enterprise setup karna hoga.
